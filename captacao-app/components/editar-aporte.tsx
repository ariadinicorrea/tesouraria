...
export function EditarAporte({ aporte, empresas, investidores, instrumentos, agentes, cautelas = [] }: {
  aporte: any;
  ...
  const [f, setF] = useState({
    codigo: aporte.codigo ?? "", empresa_id: aporte.empresa_id ?? "", investidor_id: aporte.investidor_id ?? "", instrumento_id: aporte.instrumento_id ?? "",
    cautela_id: aporte.cautela_id ?? "", data_aporte: (aporte.data_aporte ?? "").slice(0, 10),
    valor_aporte: aporte.valor_aporte != null ? String(aporte.valor_aporte) : "",
    tipo_remuneracao: aporte.tipo_remuneracao ?? "percentual_cdi",
    taxa_valor: aporte.taxa_valor != null ? String(aporte.taxa_valor) : "", periodo_taxa: aporte.periodo_taxa ?? "anual",
    percentual_cdi: aporte.percentual_cdi != null ? String(aporte.percentual_cdi) : "",
    data_vencimento: (aporte.data_vencimento ?? "")?.slice(0, 10) ?? "",
    quantidade_cotas: aporte.quantidade_cotas != null ? String(aporte.quantidade_cotas) : "",
    valor_cota_aporte: aporte.valor_cota_aporte != null ? String(aporte.valor_cota_aporte) : "",
    agente_id: aporte.agente_id ?? "",
    comissao_percentual: aporte.comissao_percentual != null ? String(aporte.comissao_percentual * 100) : "",
  });
  ...
  async function salvar() {
    ...
    try {
      const res = await fetch(`/api/aportes/${aporte.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(f) });
  ...
  return (
    <div className="rounded-lg border bg-surface p-5">
      ...
      {ok && <p className="mt-3 text-sm text-accent">Alterações salvas ✓</p>}
      <div className="mt-4 flex items-center gap-3">
        <button onClick={salvar} disabled={salvando} className="rounded-md bg-ink px-4 py-2 text-sm font-medium text-white disabled:opacity-50">{salvando ? "Salvando…" : "Salvar alterações"}</button>
        <Link href={`/aportes/${aporte.id}`} className="rounded-md border px-4 py-2 text-sm hover:bg-paper">Cancelar</Link>
      </div>
    </div>
  );
}
