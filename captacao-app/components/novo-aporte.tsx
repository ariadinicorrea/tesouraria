...
export function NovoAporte({ empresas, investidores, instrumentos, agentes, cautelas = [] }: {
  ...
  const somaCautelas = useMemo(
    () => cautelasEmpresa.filter((c) => selCautelas.includes(c.id)).reduce((s, c) => s + Number(c.valor ?? 0), 0),
    [cautelasEmpresa, selCautelas]
  );

  function toggleCautela(id: string) {
    setSelCautelas((arr) => arr.includes(id) ? arr.filter((x) => x !== id) : [...arr, id]);
  }
  function trocarEmpresa(id: string) { set("empresa_id", id); setSelCautelas([]); }

  async function salvar() {
    setSalvando(true); setErro(null); setOk(false);
    if (ehSecuritizadora && selCautelas.length === 0) {
      setSalvando(false);
      return setErro("Selecione ao menos uma cautela para a securitizadora.");
    }
    const payload = {
      ...f,
      valor_aporte: ehSecuritizadora ? String(somaCautelas) : f.valor_aporte,
      cautela_ids: ehSecuritizadora ? selCautelas : [],
    };
    ...
    <div className="rounded-lg border bg-surface p-5">
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <div>
          ...
          {ehSecuritizadora ? (
            <input className={input + " bg-paper/60"} value={fmtBRLloc(somaCautelas)} readOnly title="Calculado pela soma das cautelas selecionadas" />
          ) : (
